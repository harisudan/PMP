...
// Under different namespace than MujocoClient.exe to enable compilation together.
// 
namespace MujocoClientExtraCredit
{
	...
	void main(int argc, char* argv[])
	{
		...
		int NIterations = argc > 1 ? atoi(argv[1]) : 60;
		...
		if (mjIsConnected() && mjIsModel())
		{
			...
			printf("Running the version which sets the paddle to the anticipated target coordinate for the ball, instead of current estimated position.\n");
			...
			for (int trial = 0; trial < NUM_TRIAL; trial++)
			{
				...
				// use notation similar to tutorial for Pk
				// 
				Matrix Pk = P;
				...
				for (int k = 0;; k++)
				{
					...
					for (int i = 0; i < NIterations; i++)
					{
						...
						// A threshold of 1e-6 used to stop on further iterations
						// 
						if (diff < 0.000001)
						{
							break;
					...
					// 
					double timeToImpact = abs(xa[1] / xa[4]);

					// convert timeToImpact's units
					// 
					...
					// by using the timeToImpact, zPos, zVel and accelaration due to gravity (-2 in z direction).
					// Use the equation of motion zPosTarget = zCurrentPos + (zVel * timeToImpact) + (1/2 * acceleration * square(timeToImpact)).
					// 
					double zHeightAtImpact = xa[2] + (xa[5] * timeToImpact) + (0.5 * -2 * (timeToImpact * timeToImpact));

					// Do similar calculation to obtain x coordinate at impact - but the acceleration across x-direction is zero.
					// Use the equation of motion xPosTarget = xCurrentPos + (xVel * timeToImpact).
					// 
